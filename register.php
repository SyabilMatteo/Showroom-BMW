...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['profilePic']['name'])) {
        ...
        $foto = $targetDir . basename($_FILES["profilePic"]["name"]);
        move_uploaded_file($_FILES["profilePic"]["tmp_name"], $foto);
    }

    $query = "INSERT INTO pengguna (nama, email, password, foto_profil, no_telepon, alamat, tanggal_daftar)
              VALUES ('$nama', '$email', '$password', '$foto', '$phone', '$alamat', NOW())";
...
<body class="min-h-screen w-full flex">
  ...
  <div class="relative flex flex-col justify-start items-center w-full md:w-1/2 bg-white px-10 md:px-20 overflow-y-auto min-h-screen scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
    ...
    <div class="mt-32 flex flex-col justify-start items-center w-full max-w-md">
      ...
      <form class="w-full max-w-md pb-10" method="POST" enctype="multipart/form-data">
          <div class="mb-4">
              <label for="username" class="block mb-2 text-sm font-medium opacity-60 text-gray-600">Username</label>
              <input type="text" id="username" name="username" required placeholder="Justice Suelai" class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500">
          </div>
        ...
        <div class="flex flex-col md:flex-row md:gap-4 mb-4">
            <div class="w-full">
                ...
                <input type="email" id="email" name="email" required placeholder="justice@email.com" class="w-full p-3 border border-gray-300 rounded-md focus:ring-2  focus:ring-blue-500">
            </div>
            <div class="w-full">
                <label for="phone" class="block mb-2 text-sm font-medium opacity-60 text-gray-600">Phone</label>
                <input type="tel" id="phone" name="phone" required placeholder="+62 812 3456 7890" class="w-full p-3 border border-gray-300 rounded-md focus:ring-2  focus:ring-blue-500">
            </div>
        </div>

          <div class="mb-4">
              <label for="password" class="block mb-2 text-sm font-medium opacity-60 text-gray-600">Password</label>
              <input type="password" id="password" name="password" required placeholder="********" class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500">
          </div>

          <div class="mb-4">
              <label for="address" class="block mb-2 text-sm font-medium opacity-60 text-gray-600">Address</label>
              <textarea id="address" name="address" rows="3" required placeholder="Jl. Merdeka No. 45, Jakarta" class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 resize-none"></textarea>
          </div>

          <div class="mb-6">
              <label class="block mb-2 text-sm font-medium opacity-60 text-gray-600">Profile Picture</label>
              <input type="file" id="profilePic" name="profilePic" accept="image/*" class="w-full p-2 border border-gray-300 rounded-md text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-[#1C69D4] file:text-white hover:file:bg-[#213120] transition-colors duration-300 cursor-pointer">
          </div>
